...
const Battle: React.FC<BattleProps> = ({ route }) => {
  ...
  const startBattle = () => {
    ...
    if (ownTurn) startOwnTurn({ ...newState });
    else startEnemyTurn({ ...newState });
  };
  ...
  const startOwnTurn = (currentState: BattleState) => {
    ...
    if (newState.roundNumber > 1) {
      setBattleState({
        ...newState,
        ownMana: newState.ownMana + 1,
        ownBattlePoints: newState.ownBattlePoints + 1,
      });
    }
  ...
  const playEnemyTurn = (currentState: BattleState) => {
    const allEnemyCards: CardInterface[] = [
      ...enemyCardItems.map(item => item.card),
      enemyLeaderItem.card,
    ];
    ...
    for (const card of allEnemyCards) {
      for (const spell of card.spells) {
        if (spell.manaCost <= currentState.enemyMana) {
          cardsAndSpellsAvailable.push({ card, spell });
        } else cardsWithNotAvailableSkill = true;
      }
    }
    const cardsWithAvailableSkill = allEnemyCards.filter(
      card =>
        card.spells.filter(spell => spell.manaCost <= currentState.enemyMana)
          .length > 0,
    );
    ...
    const restOfCards = allEnemyCards.filter(
      card => !cardsAndSpellsToUse.map(obj => obj.card).includes(card),
    );
    const restCardsSortedByAttack = restOfCards.sort(
      (a, b) => b.attack - a.attack,
    );
    let cardsToAttack: CardInterface[] = [];
    if (restCardsSortedByAttack.length > 0) {
      const lastInd =
        restCardsSortedByAttack.length > currentState.enemyBattlePoints
          ? currentState.enemyBattlePoints
          : restCardsSortedByAttack.length;
      cardsToAttack = restCardsSortedByAttack.slice(0, lastInd);
    }
    const allCardsAndSpells: CardAndSpell[] = [
      ...cardsAndSpellsToUse,
      ...cardsToAttack.map(card => {
        return { card, spell: undefined };
      }),
    ];
    performEnemyAutoActionsRecursive(
      currentState,
      allCardsAndSpells,
      0,
      endEnemyTurn,
  ...
  const performEnemyAutoActionsRecursive = (
    currentState: BattleState,
    cardsAndSpells: CardAndSpell[],
    currentIndex: number,
    callback: (state: BattleState) => void,
  ) => {
  ...
  const performAction = (
    ...
    if (activeSpell) {
      ...
      for (const effect of activeSpell.effects) {
        const effectFunction = effectFunctions.find(
          func => func.id === effect.manipulateFunctionId,
        )?.effectFunction;
        if (effectFunction)
          newState = effectFunction(newState, findCardAndChangeHp);
      }
  ...
  const checkBattleHooks = (
    hook: EffectLaunchHook,
    currentState: BattleState,
    enemy: boolean,
  ) => {
    let newState = enemy
      ...
      : { ...currentState };
    for (const spell of newState.ownLeader.spells) {
      for (const effect of spell.effects) {
        if (effect.effectLaunchHook === hook) {
          ...
          if (effectFunction)
            newState = effectFunction.effectFunction(
              newState,
              findCardAndChangeHp,
            );
        }
    ...
    for (const card of newState.allies) {
      for (const spell of card.spells) {
        for (const effect of spell.effects) {
          if (effect.effectLaunchHook === hook) {
            const effectFunction = effectFunctions.find(
  ...
  const findCardAndChangeHp = (
    ...
    if (allyCard) {
      ...
      if (allyCard.hp <= 0) {
        ...
        newBattleState.allies = [
          ...newBattleState.allies.filter(ally => ally !== allyCard),
        ];
      }
      setAnimation();
    }
    if (card === newBattleState.ownLeader) {
      setDamagedCardWithTimeout({
        card: newBattleState.ownLeader,
        damage: amount,
      });
    ...
    if (card === newBattleState.enemyLeader) {
      setDamagedCardWithTimeout({
        card: newBattleState.enemyLeader,
        damage: amount,
      });
    ...
    if (enemyCard) {
      ...
      if (enemyCard.hp <= 0) {
        ...
        newBattleState.enemies = newBattleState.enemies.filter(
          enemy => enemy !== enemyCard,
        );
      }
      setAnimation();
    }
  ...
  const onOwnLeaderPress = () => {
    ...
    } else if (allyLeaderItem.active) {
      const newState = performAction({
        ...battleState,
        targets: [allyLeaderItem.card],
      });
      setBattleState({
        ...newState,
        active: undefined,
  ...
  const onEnemyLeaderPress = () => {
    ...
    } else if (enemyLeaderItem.active) {
      const newState = performAction({
        ...battleState,
        targets: [enemyLeaderItem.card],
      });
      setBattleState({
        ...newState,
        active: undefined,
  ...
  const primaryButtonTitle =
    battleState.active && ownTurn ? 'Cancel' : 'Finish turn';
  const resign = () => {
    console.log('Resigned!');
  };
  const allyModuleCard =
    moduleCard &&
    (moduleCard === allyLeaderItem.card ||
      ownCardItems.map(item => item.card).includes(moduleCard));
  const canAttack =
    battleState.ownBattlePoints > 0 &&
    allyModuleCard &&
    ownTurn &&
    ...
    <View style={{ flex: 1, justifyContent: 'center' }}>
      ...
      >
        <Image
          ...
          source={require('../assets/general/battle_field1.jpg')}
        />
      </View>
      <View
        style={{
      ...
      >
        ...
        >
          <View style={{ aspectRatio: 1, height: '100%', borderRadius: 10 }}>
            <Image
              style={{
                flex: 1,
                width: undefined,
              ...
              source={require('../assets/general/sword1.png')}
            />
          </View>
          <Text
            style={{ fontSize: 15, fontWeight: 'bold', color: COLORS.white }}
          >
            {battleState.enemyBattlePoints}
          </Text>
          <View
            style={{
              aspectRatio: 1,
          ...
          >
            <Image
              ...
              source={require('../assets/general/mana.jpg')}
            />
          </View>
          <Text
            style={{ fontSize: 15, fontWeight: 'bold', color: COLORS.white }}
          >
            {battleState.enemyMana}
          </Text>
        </View>
        <View
          style={{
        ...
        >
          ...
          >
            <View style={{ aspectRatio: 2 / 3, height: '70%' }}>
              {battleState.deadEnemies.map(enemy => (
                <View
                  key={enemy.id}
                  style={{
                    position: 'absolute',
            ...
            <View style={{ aspectRatio: 2 / 3, height: '100%' }}>
              <OpenedCard
                card={enemyLeaderItem.card}
                onPress={onEnemyLeaderPress}
                damageTaken={enemyLeaderItem.damage}
                activeSpell={enemyLeaderItem.activeSpell}
                disabled={false}
                borderColor={getCardBorderColor(enemyLeaderItem)}
              />
            </View>
          </View>
        </View>
        <View style={{ flex: 1 }}>
          <CardRow
            cardItems={enemyCardItems}
            gap={rowGap}
            onCardPress={(card: CardInterface) => {
              onEnemyCardPress(card);
            }}
            getBorderColor={getCardBorderColor}
          />
        </View>
        <View style={{ flex: 1 }}>
          <CardRow
            cardItems={ownCardItems}
            gap={rowGap}
            onCardPress={(card: CardInterface) => {
              onAllyCardPress(card);
            }}
            getBorderColor={getCardBorderColor}
          />
        </View>
        ...
        >
          ...
          >
            ...
            >
              <OpenedCard
                card={allyLeaderItem.card}
                damageTaken={allyLeaderItem.damage}
                activeSpell={allyLeaderItem.activeSpell}
                onPress={onOwnLeaderPress}
                disabled={false}
                borderColor={getCardBorderColor(allyLeaderItem)}
              />
            </View>
            <View style={{ height: '70%', aspectRatio: 2 / 3 }}>
              {battleState.deadAllies.map(ally => (
                <View
                  key={ally.id}
                  style={{
                    position: 'absolute',
        ...
        >
          ...
          >
            ...
            <Text
              style={{ fontSize: 30, fontWeight: 'bold', color: COLORS.white }}
            >
              {ownTurn ? 'Your turn' : "Enemy's turn"}
            </Text>
          </View>
        </View>
      )}
